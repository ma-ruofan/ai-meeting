...
- 创建会议、邀请加入、主持人与参会者权限；令牌只以哈希形式存入数据库。
...
3. 每位参与者用自己的加入身份，在“参会者与声纹”登记本人音频。支持上传1～3段；建议使用独立清晰语音，另留测试录音。
...
朋友设备上的普通局域网 HTTP 页面用于查看和上传文件；网页麦克风需要 localhost 或受信任 HTTPS。可在主电脑借用不同浏览器身份逐人登记录音，或让朋友上传自己的录音文件。局域网不可达时检查防火墙、访客 Wi-Fi 隔离和主机 IP。
