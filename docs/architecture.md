...
最终答案允许空引用，保留模型原文并标记成功；空答案、未读取的引用和跨会议引用仍拒绝。问候/一般知识可直接回答，相关会议事实通过提示词要求先检索，缺少记录时可说明信息缺口后给出通用建议。该策略同时用于私聊和语音，但各自的数据源隔离不变。是否检索和事实措辞依赖模型表现，需真实模型评测，不能把引用有效性校验当作自然语言事实正确性证明。
