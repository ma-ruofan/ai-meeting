...
test("create, join on another device, ask, adopt, edit and export", async ({
  page,
  browser,
}) => {
  const errors: string[] = [];
  ...
  await expect(guestPage.locator(".answer-card")).toHaveCount(1);
  ...
  await page.getByRole("button", { name: "复核并采纳" }).click();
  await page
    .getByRole("textbox", { name: "确认内容" })
    .fill("陈一周五前完成多设备测试");
